...
export class AmfService {
  ...
  /**
   * @param {String} value New working directory
   */
   ...
   * Otherwise it would make the application unsatble.
   * @return {Object} A reference to the child process
   */
      ...
      const callbacks = {
        onmessage: (result) => {
          ...
          if (result.error) {
            ...
            try {
              result.api = JSON.parse(result.api);
            } catch (e) {
              console.error(e);
            }
            resolve(result.api);
